...
#define  KLST_CONSOLE(...)       printf(__VA_ARGS__);
#define  KLST_CONSOLE_LN(...)       printf(__VA_ARGS__);printf(KLANG_NL);
#else
#define  KLST_CONSOLE(...)
...
#define  KLST_LOG_APS(...)       printf(__VA_ARGS__);printf(" :: ");
#else
#define  KLST_LOG_APS(...)
#endif
...
size_t SerialProxy::println(const String &s) {
    KLST_LOG_APS("<println>");
    KLST_LOG_APS("string");
    KLST_CONSOLE_LN("String: %s", s.c_str());
    return 0;
