...
void Echo::Tempo2Delay(int value) {
    ...
    ;
    if ((unsigned int)delay > (SAMPLE_RATE * MAX_DELAY))
        delay = SAMPLE_RATE * MAX_DELAY;
    initdelays();
}
...
void Echo::setpreset(int npreset) {
...
void Echo::changepar(int npar, int value) {
    switch (npar) {
        case 0:
            setvolume(value);
            break;
