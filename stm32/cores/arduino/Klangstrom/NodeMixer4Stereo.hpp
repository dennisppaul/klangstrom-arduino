    ...
    public:
        ...
        void update(CHANNEL_ID pChannel, SIGNAL_TYPE* pAudioBlock) {
            if (is_not_updated()
                ...
                const bool b0 = (mBlockData_SIGNAL_0!=nullptr);
                const bool b1 = (mBlockData_SIGNAL_1!=nullptr);
                const bool b2 = (mBlockData_SIGNAL_2!=nullptr);
                const bool b3 = (mBlockData_SIGNAL_3!=nullptr);
                const float pL0 = mPan[0];
                const float pR0 = 1.0 - mPan[0];
                const float pL1 = mPan[1];
                const float pR1 = 1.0 - mPan[1];
                const float pL2 = mPan[2];
                const float pR2 = 1.0 - mPan[2];
                const float pL3 = mPan[3];
                const float pR3 = 1.0 - mPan[3];
                const uint8_t mSignalInputCounter = (b0 ? 1 : 0) +
                                                    (b1 ? 1 : 0) +
                                                    (b2 ? 1 : 0) +
                                                    (b3 ? 1 : 0);
