...
#define __SLAVE_SELECT(d, s)                \
    if (__IS_SPI_SLAVE(s)) {                \
        ili9341_spi_slave_select((d), (s)); \
    }

#define __SLAVE_RELEASE(d, s)                \
    if (__IS_SPI_SLAVE(s)) {                 \
        ili9341_spi_slave_release((d), (s)); \
    }
...
void ili9341_calibrate_3point(ili9341_t *lcd,
    ...
    //     (float)(((screen_a_x - screen_c_x) * (touch_b_y - touch_c_y)) -
    //             ((screen_b_x - screen_c_x) * (touch_a_y - touch_c_y))) /
    //     delta;

    // lcd->touch_3point.beta_x =
    //     (float)(((touch_a_x - touch_c_x) * (screen_b_x - screen_c_x)) -
    //             ((touch_b_x - touch_c_x) * (screen_a_x - screen_c_x))) /
    //     delta;

    // lcd->touch_3point.delta_x =
    //     (float)((((int64_t)screen_a_x * ((touch_b_x * touch_c_y) - (touch_c_x * touch_b_y))) -
    ...
    //     (float)(((screen_a_y - screen_c_y) * (touch_b_y - touch_c_y)) -
    //             ((screen_b_y - screen_c_y) * (touch_a_y - touch_c_y))) /
    //     delta;

    // lcd->touch_3point.beta_y =
    //     (float)(((touch_a_x - touch_c_x) * (screen_b_y - screen_c_y)) -
    //             ((touch_b_x - touch_c_x) * (screen_a_y - screen_c_y))) /
    //     delta;

    // lcd->touch_3point.delta_y =
    //     (float)((((int64_t)screen_a_y * (touch_b_x * touch_c_y - touch_c_x * touch_b_y)) -
...
static void ili9341_initialize(ili9341_t *lcd) {
    ...
    ili9341_spi_write_command_data(lcd, issNONE,
                                   0xE0, 15, (uint8_t[]){0x0F, 0x31, 0x2B, 0x0C, 0x0E, 0x08, 0x4E, 0xF1, 0x37, 0x07, 0x10, 0x03, 0x0E, 0x09, 0x00});

    // NEGATIVE GAMMA CORRECTION
    ili9341_spi_write_command_data(lcd, issNONE,
                                   0xE1, 15, (uint8_t[]){0x00, 0x0E, 0x14, 0x03, 0x11, 0x07, 0x31, 0xC1, 0x48, 0x08, 0x0F, 0x0C, 0x31, 0x36, 0x0F});
