...
uint16_t spi_tx_block[__SPI_TX_BLOCK_MAX__]
#ifdef ILI9341_USE_DMA
__attribute__((section(".dma_buffer")))
#endif
;  // default DMA TX buffer
...
void ili9341_spi_tft_set_address_rect(ili9341_t *lcd,
    ...
    ili9341_spi_write_command_data(lcd, issNONE,
                                   0x2A, 4, (uint8_t[]){(x0 >> 8) & 0xFF, x0 & 0xFF, (x1 >> 8) & 0xFF, x1 & 0xFF});

    // row address set
    ili9341_spi_write_command_data(lcd, issNONE,
                                   0x2B, 4, (uint8_t[]){(y0 >> 8) & 0xFF, y0 & 0xFF, (y1 >> 8) & 0xFF, y1 & 0xFF});
...
#else
    HAL_SPI_Transmit(lcd->spi_hal, (uint8_t *)color, size, 128);
#endif
