...
void ProcessReceivedMidiDatas(void) {
    ...
    while (numberOfPackets--) {
        ...
        EventMIDI mEvent = {cable, command, channel, a, b};

        switch (command) {
            case MIDI_NOTE_OFF:
                receive_midi_note_off(channel, a);
                USBH_DbgLog("receive_midi_note_off");
                break;
            case MIDI_NOTE_ON:
                receive_midi_note_on(channel, a, b);
                USBH_DbgLog("receive_midi_note_on");
                break;
            case MIDI_AFTERTOUCH:
                receive_midi_aftertouch(channel, a, b);
                break;
            case MIDI_CONTROL_CHANGE:
                receive_midi_control_change(channel, a, b);
                break;
            case MIDI_PROGRAM_CHANGE:
                receive_midi_program_change(channel, a);
                break;
            case MIDI_CHANNEL_PRESSURE:
                receive_midi_channel_pressure(channel, a);
                break;
            case MIDI_PITCHBEND:
                receive_midi_pitch_bend(channel, (int16_t)(((a << 7) + b) - 0x2000));
                break;
            case MIDI_SYS_EX:
                receive_midi_sys_ex(channel, a, b);
                break;
            default:
                receive_midi_message(pack.cin_cable, pack.evnt0, pack.evnt1, pack.evnt2);
        }
    }
}
